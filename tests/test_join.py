...
def test_catalytic_core_is_not_called_divergent():
    ...
    out = build(
        [site(10, "type1", 0.9), site(10, "type2", 0.8)],
...
def test_missing_conservation_is_surfaced_not_dropped():
    out = build([site(50, "type2", 0.001)], [cons(50, "all", 0.2)])
    assert len(out) == 1
...
def test_qvalue_preferred_over_pvalue():
    sites = pd.DataFrame([site(70, "type2", 0.001)])
    sites["qvalue"] = [0.9]
    out = classify(
        sites,
...
def test_every_tested_column_appears_once():
    sites = [site(c, "type2", 0.5) for c in range(1, 6)]
    conservation = [
...
def test_summary_counts_classes():
    out = build(
        [site(10, "type2", 0.9), site(20, "type2", 0.001)],
        [
            cons(10, "all", 0.05), cons(10, "A", 0.05), cons(10, "B", 0.05),
            cons(20, "all", 1.2), cons(20, "A", 0.1), cons(20, "B", 0.1),
        ],
